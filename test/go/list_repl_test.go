...
func TestListRepl(t *testing.T) {
	...
	t.Run("PrintMultiple", TestListReplPrintMultiple)
...
// Tests list_print with multiple elements
func TestListReplPrintMultiple(t *testing.T) {
	inputWriter, output := startListRepl(t)

	//push multiple links onto head of list
	io.WriteString(inputWriter, "list_push_head 1\n")
	io.WriteString(inputWriter, "list_push_head 2\n")
	io.WriteString(inputWriter, "list_push_head 3\n")

	//call list_print and check for valid response
	io.WriteString(inputWriter, "list_print\n")
	checkOutputExact(t, output, "3\n2\n1\n")
}
